...
- `src/interactions.ts` — All `/disclaw` slash command handlers (config, clear, interrupt, model, fork, resume) and `validateWorkingDir`. Thread-only commands use `requireThreadSession()` guard
...
- `src/claude-client.ts` — SDK wrapper; calls `query()` from `@anthropic-ai/claude-agent-sdk` and streams SDKMessages via `onMessage` callback. Supports `model`, `forkSession`, `resumeSessionAt`, `onQuery` options
...
**Data stores:**
- SQLite (`./data/threads.db`) — thread/session mappings (incl. model), channel configs
...
Required: `DISCORD_BOT_TOKEN`. Optional: `CLAUDE_WORKING_DIR`, `DISCLAW_ALLOWED_DIRS` (comma-separated security allowlist), `DB_PATH` (default: `./data/threads.db`), `TZ`. See `.env.example`.
